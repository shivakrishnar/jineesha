...
export async function getEsignatureAppByCompany(tenantId: string, companyId: string, token: string): Promise<EsignatureAppInfo> {
    ...
    console.log(`tenantId: ${tenantId}`, `companyId ${companyId}`, `token: ${token}`);
