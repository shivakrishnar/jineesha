...
describe('notification.service.reminder', () => {
    beforeEach(() => {
        setup();
        (jwt.decode as jest.Mock).mockImplementation(() => ({ applicationId: '123456' }));
    });

    test('successfully send reminder email to employee', async () => {
        ...
        return await notificationService.processEvent(mockData.reminderEvent).then((response) => {
