...
describe('notification.service.delete', () => {
    beforeEach(() => {
        setup();
        (jwt.decode as jest.Mock).mockImplementation(() => ({ applicationId: '123456' }));
    });

    test('successfully send pending signature cancellation email to employee', async () => {
        ...
        return await notificationService.processEvent(mockData.deleteEvent).then((response) => {
