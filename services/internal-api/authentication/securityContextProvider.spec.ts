        ...
        (jwt.decode as jest.Mock).mockImplementation((token) =>
            token === 'ahr-token'
                ? {
                      header: {},
                      payload: {
                          applicationId: 'ahr-id',
                          account: { id: 'acct-id' },
                          scope: ['prefix/ahr-role'],
                ...
                ? {
                      header: {},
                      payload: { applicationId: 'other-id', account: { id: 'acct-id' }, scope: ['prefix/other-role'], policy: {} },
                  }
                : token === 'v2-token'
                ? {
                      header: { kid: 'key-id' },
                      payload: {
                          account: { id: 'acct-id' },
                          scope: ['prefix/ahr-role', 'prefix/other-role'],
                          policy: { rules: [{ action: 'test' }] },
                      },
                  }
                : undefined,
    ...
    describe('if anonymous not allowed', () => {
        ...
        describe('with v1 token', () => {
            test('throws 401 if token cannot be verified', async () => {
                ...
                await expect(sut.getSecurityContext({ event: { headers: { authorization: 'Bearer ahr-token' } } })).rejects.toMatchObject({
                    statusCode: 401,
                });
            });
        ...
        describe('with v2 token', () => {
            test('throws 401 if public key not found', async () => {
                ...
                await expect(sut.getSecurityContext({ event: { headers: { authorization: 'Bearer v2-token' } } })).rejects.toMatchObject({
                    statusCode: 401,
                });
            });
