...
describe('deleteApplicationStatusHistory', () => {

    test('companyId must be an integer', () => {
        return ApplicationStatusHistoryService.deleteApplicationStatusHistory(
            sharedMockData.tenantId, 
            sharedMockData.companyIdWithCharacter,
            sharedMockData.userEmail,
            mockData.ApplicationStatusHistoryToDeleteId,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
    ...
    test('id must be an integer', () => {
        return ApplicationStatusHistoryService.deleteApplicationStatusHistory(
            ...
            sharedMockData.companyId,
            sharedMockData.userEmail,
            mockData.ApplicationStatusHistoryToDeleteIdWithCharacter,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
                expect(error.message).toEqual('The provided request object was not valid for the requested operation.');
                expect(error.developerMessage).toEqual(`${mockData.ApplicationStatusHistoryToDeleteIdWithCharacter} is not a valid number`);
                expect(error.moreInfo).toEqual('');
            });
    });
    ...
    test('deletes ApplicationStatusHistory', async () => {
        (utilService as any).invokeInternalService = jest.fn(async(transaction, payload) => {
            if (payload.queryName === 'deleteApplicationStatusHistory'){
                return true;
            } else if (payload.queryName === 'getApplicationStatusHistoryById') {
                const result = await Promise.resolve(mockData.getApplicationStatusHistoryByIdDBResponse);
                return result;
        ...
        return await ApplicationStatusHistoryService
            ...
            .then((result) => {
                expect(result).toEqual(mockData.deleteApplicationStatusHistoryAPIResponse);
            });
    });
});
