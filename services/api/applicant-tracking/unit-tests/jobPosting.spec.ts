...
describe('deleteJobPosting', () => {
    ...
    test('companyId must be an integer', () => {
        return jobPostingService.deleteJobPosting(
            sharedMockData.tenantId, 
            sharedMockData.companyIdWithCharacter,
            sharedMockData.userEmail,
            mockData.jobPostingToDeleteId,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
    ...
    test('id must be an integer', () => {
        return jobPostingService.deleteJobPosting(
            ...
            sharedMockData.companyId,
            sharedMockData.userEmail,
            mockData.jobPostingToDeleteIdWithCharacter,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
                expect(error.message).toEqual('The provided request object was not valid for the requested operation.');
                expect(error.developerMessage).toEqual(`${mockData.jobPostingToDeleteIdWithCharacter} is not a valid number`);
                expect(error.moreInfo).toEqual('');
            });
    });
    ...
    test('The requested resource must exist', async () => {
        (utilService as any).invokeInternalService = jest.fn(async(transaction, payload) => {
            if (payload.queryName === 'getJobPostingById') {
                const result = await Promise.resolve(mockData.getJobPostingByIdDBResponseEmpty);
                return result;
            } else {
                return {};
            }
    ...
    test('deletes JobPosting', async () => {
        (utilService as any).invokeInternalService = jest.fn(async(transaction, payload) => {
            if (payload.queryName === 'deleteJobPosting'){
                return true;
            } else if (payload.queryName === 'getJobPostingById') {
                const result = await Promise.resolve(mockData.getJobPostingByIdDBResponse);
                return result;
        ...
        return await jobPostingService
            ...
            .then((result) => {
                expect(result).toEqual(mockData.deleteJobPostingAPIResponse);
            });
    });
});
