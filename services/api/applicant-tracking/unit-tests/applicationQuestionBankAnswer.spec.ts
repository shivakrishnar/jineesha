...
describe('deleteApplicationQuestionBankAnswer', () => {

    test('companyId must be an integer', () => {
        return ApplicationQuestionBankAnswerService.deleteApplicationQuestionBankAnswer(
            sharedMockData.tenantId, 
            sharedMockData.companyIdWithCharacter,
            sharedMockData.userEmail,
            mockData.ApplicationQuestionBankAnswerToDeleteId,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
    ...
    test('id must be an integer', () => {
        return ApplicationQuestionBankAnswerService.deleteApplicationQuestionBankAnswer(
            ...
            sharedMockData.companyId,
            sharedMockData.userEmail,
            mockData.ApplicationQuestionBankAnswerToDeleteIdWithCharacter,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
                expect(error.message).toEqual('The provided request object was not valid for the requested operation.');
                expect(error.developerMessage).toEqual(`${mockData.ApplicationQuestionBankAnswerToDeleteIdWithCharacter} is not a valid number`);
                expect(error.moreInfo).toEqual('');
            });
    });
    ...
    test('deletes ApplicationQuestionBankAnswer', async () => {
        (utilService as any).invokeInternalService = jest.fn(async(transaction, payload) => {
            if (payload.queryName === 'deleteApplicationQuestionBankAnswer'){
                return true;
            } else if (payload.queryName === 'getApplicationQuestionBankAnswerById') {
                const result = await Promise.resolve(mockData.getApplicationQuestionBankAnswerByIdDBResponse);
                return result;
        ...
        return await ApplicationQuestionBankAnswerService
            ...
            .then((result) => {
                expect(result).toEqual(mockData.deleteApplicationQuestionBankAnswerAPIResponse);
            });
    });
});
