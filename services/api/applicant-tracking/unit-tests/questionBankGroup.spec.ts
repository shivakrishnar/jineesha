...
describe('deleteQuestionBankGroup', () => {

    test('companyId must be an integer', () => {
        return questionBankGroupService.deleteQuestionBankGroup(
            sharedMockData.tenantId, 
            sharedMockData.companyIdWithCharacter,
            sharedMockData.userEmail,
            mockData.QuestionBankGroupToDeleteId,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
    ...
    test('id must be an integer', () => {
        return questionBankGroupService.deleteQuestionBankGroup(
            ...
            sharedMockData.companyId,
            sharedMockData.userEmail,
            mockData.QuestionBankGroupToDeleteIdWithCharacter,
            ).catch((error) => {
                expect(error).toBeInstanceOf(ErrorMessage);
                expect(error.statusCode).toEqual(400);
                expect(error.code).toEqual(30);
                expect(error.message).toEqual('The provided request object was not valid for the requested operation.');
                expect(error.developerMessage).toEqual(`${mockData.QuestionBankGroupToDeleteIdWithCharacter} is not a valid number`);
                expect(error.moreInfo).toEqual('');
            });
    });
    ...
    test('The requested resource must exist', async () => {
        (utilService as any).invokeInternalService = jest.fn(async(transaction, payload) => {
            if (payload.queryName === 'getQuestionBankGroupById') {
                const result = await Promise.resolve(mockData.getQuestionBankGroupByIdDBResponseEmpty);
                return result;
            } else {
                return {};
            }
    ...
    test('deletes QuestionBankGroup', async () => {
        (utilService as any).invokeInternalService = jest.fn(async(transaction, payload) => {
            if (payload.queryName === 'deleteQuestionBankGroup'){
                return true;
            } else if (payload.queryName === 'getQuestionBankGroupById') {
                const result = await Promise.resolve(mockData.getQuestionBankGroupByIdDBResponse);
                return result;
        ...
        return await questionBankGroupService
            ...
            .then((result) => {
                expect(result).toEqual(mockData.deleteQuestionBankGroupAPIResponse);
            });
    });
});
