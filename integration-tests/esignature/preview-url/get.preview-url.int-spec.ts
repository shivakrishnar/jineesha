...
describe('get preview by tenant', () => {
    ...
    test('must return a 400 if tenantID is invalid', (done) => {
        ...
        const uri = `/tenants/${invalidTenantId}/documents/${configs.esignature.documentId}/preview`;
        request(baseUri)
            .get(uri)
            .set('Authorization', `Bearer ${accessToken}`)
            .expect(utils.corsAssertions(configs.corsAllowedHeaderList))
    ...
    test.skip('must return a 404 if tenantID is not found', (done) => {
        ...
        const uri = `/tenants/${unknownTenantId}/documents/${configs.esignature.documentId}/preview`;
        request(baseUri)
            .get(uri)
            .set('Authorization', `Bearer ${accessToken}`)
            .expect(utils.corsAssertions(configs.corsAllowedHeaderList))
...
describe('get preview by company', () => {
    ...
    test('must return a 400 if tenantID is invalid', (done) => {
        const invalidTenantId = '99999999';
        const uri = `/tenants/${invalidTenantId}/companies/${configs.companyId}/documents/${configs.esignature.documentId}/preview`;
        request(baseUri)
            .get(uri)
            .set('Authorization', `Bearer ${accessToken}`)
            .expect(utils.corsAssertions(configs.corsAllowedHeaderList))
    ...
    test.skip('must return a 404 if tenantID is not found', (done) => {
        const unknownTenantId = uuidV4();
        const uri = `/tenants/${unknownTenantId}/companies/${configs.companyId}/documents/${configs.esignature.documentId}/preview`;
        request(baseUri)
            .get(uri)
            .set('Authorization', `Bearer ${accessToken}`)
            .expect(utils.corsAssertions(configs.corsAllowedHeaderList))
