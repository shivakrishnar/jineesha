...
const updateLicenseEmailAcknowledgedSchema = JSON.parse(
    fs.readFileSync('services/api/models/updateLicenseEmailAcknowledged.json').toString(),
);
const schemas = [errorMessageSchema, updateLicenseEmailAcknowledgedSchema];
...
describe('update employee license EmailAcknowledged column By Id', () => {
    ...
    test('must return 200 if emailAcknowledged is patched', (done) => {
        ...
        request(baseUri)
            ...
            .end((error, response) => {
                utils.testResponse(error, response, done, () => {
                    return utils.assertJsonOrThrow(schemas, updateLicenseEmailAcknowledgedSchema, response.body);
                });
